...
func (c *TypeScriptGenerationContext) generateCode() (*gen.CodeGenerationResult, error) {
	...
	if c.IsSingleFile {
		...
		c.SingleFileBuilder = codeBuilder.NewCodeBuilder()
	...
	if c.IsSingleFile {
		...
		err := ssg.WriteFileStr(path, c.SingleFileBuilder.String(nil))
		if err != nil {
			return nil, err
		}
		...
		for modelName, builder := range c.ModelClasses {
			...
			// In multi-file mode, we need to add imports to the top of the file
			// But since we are building the content linearly, we might have missed imports if we didn't collect them.
			// However, in TS, imports usually go at the top.
			// For simplicity in this iteration, let's assume we can just prepend imports or we handle them during generation.
			// Actually, `codeBuilder` supports `DoImport`.

			err := ssg.WriteFileStr(path, builder.String(nil))
			if err != nil {
				return nil, err
			}
			result.OutputFiles = append(result.OutputFiles, path)
